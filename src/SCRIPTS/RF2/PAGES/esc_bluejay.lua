...
local endEscEditing = function(field, page)
    ...
    rf2.useApi("mspEscBluejay").read(receivedEscParameters, page)
end

receivedEscParameters = function(page, data)
    escParameters = data
    clearForm(page)
    page.labels, page.fields = rf2.executeScript("PAGES/esc_bluejay_form", escParameters, escCount, selectedEsc, endEscEditing)
    page.readOnly = false
...
local page = {
    ...
    write = function(self)
        clearForm(self)
        rf2.useApi("mspEscBluejay").write(escParameters)
        escParameters = nil
        rf2.useApi("mspEscBluejay").read(receivedEscParameters, self)
    end,
...
page.labels, page.fields = rf2.executeScript("PAGES/esc_bluejay_form", nil)
